...
mod parser_parse {
    ...
    impl<'a> Parser<'a> {
        ...
        pub fn parse_astro(self) -> AstroParserReturn<'a> {
            ...
            let (frontmatter, frontmatter_errors) = if let Some(info) = frontmatter_info {
                ...
                let frontmatter_content = &self.source_text[info.content_start..info.content_end];
                let padding = " ".repeat(info.content_start);
                let padded_source =
                    self.allocator.alloc_str(&format!("{padding}{frontmatter_content}"));
    ...
    fn parse_scripts_in_expression<'a>(
        ...
        source_type: SourceType,
        options: ParseOptions,
        expr: &mut oxc_ast::ast::JSXExpression<'a>,
        errors: &mut Vec<OxcDiagnostic>,
    ) {
        ...
        match expr {
            ...
            JSXExpression::JSXElement(element) => {
                // Recurse into JSX element children - this is the main case we care about
                for child in &mut element.children {
                    parse_scripts_in_child(
                        allocator,
                        source_text,
    ...
    fn parse_scripts_in_inner_expression<'a>(
        ...
        source_type: SourceType,
        options: ParseOptions,
        expr: &mut oxc_ast::ast::Expression<'a>,
        errors: &mut Vec<OxcDiagnostic>,
    ) {
        use oxc_ast::ast::Expression;
    ...
    fn parse_scripts_in_argument<'a>(
        ...
        source_type: SourceType,
        options: ParseOptions,
        arg: &mut oxc_ast::ast::Argument<'a>,
        errors: &mut Vec<OxcDiagnostic>,
    ) {
    ...
    fn parse_scripts_in_statement<'a>(
        ...
        source_type: SourceType,
        options: ParseOptions,
        stmt: &mut oxc_ast::ast::Statement<'a>,
        errors: &mut Vec<OxcDiagnostic>,
    ) {
        use oxc_ast::ast::Statement;
