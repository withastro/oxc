...
impl<'a> AstroCodegen<'a> {
    ...
    fn print_astro_root(&mut self, root: &'a AstroRoot<'a>) {
        ...
        // 4. Print metadata export if enabled
    ...
    fn print_metadata(&mut self) {
        ...
        let client_only_str = if self.client_only_components.is_empty() {
            "[]".to_string()
        } else {
            let items: Vec<String> =
                self.client_only_components.iter().map(|s| format!("\"{s}\"")).collect();
            format!("[{}]", items.join(", "))
        };
                ...
                if let Statement::ImportDeclaration(import) = stmt {
                    ...
                    if import.import_kind != ImportOrExportKind::Type {
                        ...
                        if is_client_only_import {
                            // For client:only, add to clientOnlyComponents instead of modules
    ...
    fn print_component_element(&mut self, el: &JSXElement<'a>, name: &str) {
        ...
        if let Some((value, is_html, needs_unescape, is_raw_text)) = set_directive {
            let async_prefix = self.get_async_prefix();
            self.print(&format!(",{{\"default\": {async_prefix}() => "));
            self.print(runtime::RENDER);
            self.print("`");
            if is_raw_text {
    ...
    fn print_component_attributes_filtered(
        ...
        if let Some(directive) = &hydration.directive {
            ...
            // For client:only: use $$metadata.resolvePath() wrapper, no parens for export
            // For regular hydration: use plain string literal with parens
            if let Some(path) = &hydration.component_path {
                if hydration.is_client_only {
                    // client:only uses $$metadata.resolvePath for the path
                    self.print(&format!(
                        ",\"client:component-path\":($$metadata.resolvePath(\"{path}\"))"
                    ));
                } else {
                    // Regular hydration (client:load, client:visible, etc.) uses plain string with parens
    ...
    fn print_component_default_slot_only(&mut self, children: &[JSXChild<'a>]) {
        let async_prefix = self.get_async_prefix();
        self.print("{\"default\": ");
        self.print(&format!("{async_prefix}() => "));
        self.print(runtime::RENDER);
        self.print("`");
        ...
        match expr {
            ...
            Expression::JSXFragment(frag) => {
                ...
                if is_explicit_fragment {
                    ...
                    self.print("`${");
                    self.print(runtime::RENDER_COMPONENT);
                    self.print("($$result,\"Fragment\",Fragment,{},{\"default\":() => ");
                    self.print(runtime::RENDER);
                    self.print("`");
                    for child in &frag.children {
                        self.print_jsx_child(child);
