...
use oxc_astro_codegen::{AstroCodegen, HoistedScriptType, TransformOptions};
...
fn parse_scoped_style_strategy(s: Option<&str>) -> oxc_astro_codegen::ScopedStyleStrategy {
    match s {
        Some("class") => oxc_astro_codegen::ScopedStyleStrategy::Class,
        Some("attribute") => oxc_astro_codegen::ScopedStyleStrategy::Attribute,
        _ => oxc_astro_codegen::ScopedStyleStrategy::Where,
    }
}
...
fn compile_astro_impl(source_text: &str, options: &AstroCompileOptions) -> AstroCompileResult {
    ...
    let codegen_options = TransformOptions {
        ...
        render_script: options.render_script.unwrap_or(false),
    ...
    let scripts = result
        ...
        .map(|s| match s.script_type {
            HoistedScriptType::External => {
                NapiHoistedScript { script_type: "external".to_string(), src: s.src, code: None }
            }
            HoistedScriptType::Inline => {
                NapiHoistedScript { script_type: "inline".to_string(), code: s.code, src: None }
            }
        })
        .collect();
